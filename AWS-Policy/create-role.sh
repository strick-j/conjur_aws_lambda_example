...
gather_info(){
  ...
  while : ; do
    sys_read "3. Please enter the filename for your trust policy document:"
    trustPolicy=$userAnswer
    # Verify trust policy document is present
    if [ -f $PWD/$trustPolicy ]; then
      printf "  - $trustPolicy document found. Continuing...\n\n"
      break
    fi
    printf "  - $trustPolicy document not found. Please try again. [ use CTRL+C to exit ]\n\n"
  done

  while : ; do
    sys_read "4. Please enter the filename for you permissions policy document:"
    permissionsPolicy=$userAnswer
    # Verify permissions policy document is present
    if [ -f $PWD/$permissionsPolicy ]; then
      printf "  - $permissionsPolicy document found. Continuing...\n\n"
      break
    fi
    printf "  - $permissionsPolicy document not found. Please try again. [ use CTRL+C to exit ]\n\n"
  done
